			...
			<?php 
               ...
               $postImage = wp_get_attachment_url( get_post_thumbnail_id(get_the_ID()) );
				...
				if(!empty($postImage)): ?>
					...
					<div class="links">
					<a href="<?php echo esc_url($postImage); ?>" title="<?php esc_attr_e( 'Click to view Full Image', 'technivo' );?>" data-lightbox="example-set" class="icon mustang-gallery"><i class="fa fa-plus"></i></a> <a href="<?php echo esc_url(get_permalink()); ?>" title="<?php esc_attr_e( 'Click to view Read More', 'technivo' );?>" class="icon readmore"><i class="fa fa-link"></i></a> 
					</div>
				</div>
				<?php endif; ?>
			...
			<div class="entry-main-header post-header">
				<div class="blog-header"><!-- blog-header -->
						...
						<header class="entry-header <?php echo esc_attr($entry_meta_class); ?>">					
								<h3 class="entry-title"> <a href="<?php esc_url(the_permalink()); ?>" rel="bookmark">
								<?php the_title(); ?>
								</a></h3>
						</header><!-- .entry-header -->	
	...
	<div class="entry-content-other <?php if(empty($postImage)): ?> non <?php endif; ?>">		
		<?php if ( is_search() || !is_single()) : // Only display Excerpts for Search and not single pages ?>
			...
			<?php 
			 ...
			 $postImage = wp_get_attachment_url( get_post_thumbnail_id(get_the_ID()) );
			 ?>
			 <?php the_content( wp_kses( __('Continue reading <span class="meta-nav">&rarr;</span>', 'technivo' ),wntr_allowed_html())); ?>
			<?php wp_link_pages( array( 'before' => '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'technivo' ) . '</span>', 'after' => '</div>', 'link_before' => '<span>', 'link_after' => '</span>' ) ); ?>
			</div><!-- .entry-content -->	
		<?php endif; ?>
