...
/**
 * Webibazaar
 * @copyright  	   Copyright (c) Webibazaar.
...
function wntr_service_contact() {
	echo '<div class="contact-block"><span class="contact-icon"></span>&nbsp;<div class="contact-inner"><div class="contact-inner number">'.get_option('wntr_contact_phone').'</div><div class="contact-title">'.get_option('wntr_contact_phone').'</div></div></div>';
}
...
function wntr_continue_reading_link() {
	return ' <a class="read-more-link" href="'.esc_url(get_permalink()) . '">' . esc_html__( 'Подробнее', 'technivo' ) . '</a>';
}
add_filter( 'excerpt_length', 'wntr_excerpt_length' );
/**
 ...
 * @deprecated Deprecated in Winter for WordPress 3.1
 ...
 * to remove the default style. Using Winter in WordPress 3.0 will show the styles,
 * but they won't have any effect on the widget in default Winter styling.
 *
 */
...
{	
	...
	$src = ( empty ( $src ) || $src == 'image_not_specified' )  ? get_template_directory_uri()."/images/webi/placeholder.png" : $src;
	$src = wntr_mr_image_resize($src, $width, $height, true, $align, false);
	$return = "<img src='". $src ."' title='$alttext' alt='". $alttext ."' width='". $width ."' height='".$height."' />";	
...
function wntr_excerpt($limit) 
{
	...
	array_pop($excerpt);
	$excerpt = implode(" ",$excerpt).'.<div class="read-more"><a class="read-more-link" href="'.esc_url(get_permalink()).'">'.esc_html__( 'Подробнее' , 'technivo' ).'</a></div>';
	} else {
...
if ( ! function_exists( 'wntr_site_icon' ) ) {
	function wntr_site_icon() {
		...
		if ( ! function_exists( 'has_site_icon' ) || ! has_site_icon() ) {			
			$wntr_fav_site_icon = (isset($webi['wntr_fav_site_icon']) && !empty($webi['wntr_fav_site_icon']) ) ? $webi['wntr_fav_site_icon']:null;
			$fav_url = (isset($wntr_fav_site_icon['url']) && !empty($wntr_fav_site_icon['url']) ) ? $wntr_fav_site_icon['url']:get_template_directory_uri()."/winter/favicon.ico";
			echo '<link rel="shortcut icon" type="image/png" href="'.$fav_url.'" />';
		}
		else
	    {
 ...
 * Galleries are styled by the theme in Winter's style.css. This is just
 * a simple filter call that tells WordPress to not use the default styles.
 */
add_filter( 'use_default_gallery_style', '__return_false' );
/**
 * Enqueue Winter Fonts
 */
function wntr_load_fonts() {
    ...
    */
	...
	$FiraSans = _x( 'on', 'Fira Sans font: on or off', 'technivo' ); 
		if ('off' !== $FiraSans ) {
			$font_families = array(); 
			if ( 'off' !== $FiraSans ) {
...
function wntr_load_scripts() {	
		...
		wp_enqueue_script( 'slick', get_template_directory_uri() . '/js/winter/slick.min.js', array('jquery'), '', false);
		wp_enqueue_script( 'bootstrap-min', get_template_directory_uri() . '/js/winter/bootstrap.min.js', array(), '', false);
		wp_enqueue_script( 'countdown', get_template_directory_uri() . '/js/winter/jquery.countdown.min.js', array(), '', true);
...
/**
 * Enqueue Winter Control Panel Scripts
 */
...
function wntr_search_form( $form ) {
    ...
    <div><label class="screen-reader-text" for="s">' . esc_html__( 'Search for:', 'technivo' ) . '</label>
    <input class="search-field" type="text" placeholder="' . esc_attr__( 'Search..', 'technivo' ) . '" value="' . get_search_query() . '" name="s" id="s" />
    <input class="search-submit" type="submit" id="searchsubmit" value="'. esc_attr__( 'Go', 'technivo' ) .'" />
    </div>
    </form>';
    return $form;
}
...
<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
  <p>
    <?php esc_html_e( 'Pingback:', 'technivo' ); ?>
    <?php comment_author_link(); ?>
    <?php edit_comment_link( esc_html__( '(Edit)', 'technivo' ), '<span class="edit-link">', '</span>' ); ?>
  </p>
</li>
<?php
			break;
...
<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
  <div id="comment-<?php comment_ID(); ?>" class="comment-body">
    ...
    <div class="author-content">
      ...
      <?php edit_comment_link( esc_html__( 'Edit', 'technivo' ), ' ' ); ?>
      <div class="clearfix"></div>
      <abbr class="published" title="<?php echo the_title_attribute() ?>"><?php printf( esc_html__( '%1$s at %2$s', 'technivo' ), get_comment_date(),  get_comment_time() ); ?></abbr> </div>
    ...
    <?php esc_html_e( 'Your comment is awaiting moderation.', 'technivo' ); ?>
    </em> <br />
    <?php endif; ?>
  </div>
  <!-- #comment-##  -->
...
function wntr_sticky_post() {
	if ( is_sticky() && is_home() && ! is_paged() )
		echo '<div class="meta-inner"><span class="sticky-post">' . esc_html__( 'Sticky', 'technivo' ) . '</span></div>';
}
endif;
if ( ! function_exists( 'wntr_categories_links' ) ) :
function wntr_categories_links() {
	// Translators: used between list items, there is a space after the comma.
	$categories_list = get_the_category_list( esc_html__( ', ', 'technivo' ) );
	if ( $categories_list ) {
...
function wntr_tags_links() {
	// Translators: used between list items, there is a space after the comma.
	$tag_list = get_the_tag_list( '', esc_html__( ', ', 'technivo' ) );
	if ( $tag_list ) {
...
function wntr_author_link() {
	...
	if ( 'post' == get_post_type() ) {
		printf( '<div class="meta-inner"><span class="author vcard"><i class="fa fa-pencil-square-o"></i><a class="url fn n" href="%1$s" title="%2$s" rel="author">'.esc_html__("by ",'technivo').'%3$s</a></span></div>',
			esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
			esc_attr( sprintf( esc_html__( 'View all posts by %s', 'technivo' ), get_the_author() ) ),
			get_the_author()
		);
	}
}
...
function wntr_comments_link() {
	...
	comments_popup_link( esc_html__( 'Leave a Comment', 'technivo' ), esc_html__( '1 Comment', 'technivo' ), esc_html__( '% Comments', 'technivo' ) ); 
	echo '</span></div>';
	endif; 
}
endif;
...
function wntr_entry_date( $echo = true ) {
	if ( has_post_format( array( 'chat', 'status' ) ) )
		$format_prefix = _x( '%1$s on %2$s', '1: post format name. 2: date', 'technivo' );
	else
		$format_prefix = '%2$s';
...
<time class="entry-date" datetime="%3$s">%4$s</time></a></span></div>',
		...
		esc_attr( sprintf( esc_html__( 'Permalink to %s', 'technivo' ), the_title_attribute( 'echo=0' ) ) ),
...
function wntr_posts_short_description() {
	...
	if(empty($wntr_posts_short_description))
		$wntr_posts_short_description = wntr_excerpt(26);
...
function paginate_links() {
	...
	$links = paginate_links( array(
		...
		'prev_text' =>  wp_kses( __( '<i class="fa fa-angle-left"></i>', 'technivo' ),wntr_allowed_html()),
		'next_text' =>  wp_kses( __( '<i class="fa fa-angle-right"></i>', 'technivo' ),wntr_allowed_html()),
	) );
	if ( $links ) :
...
add_image_size( 'wntr-blog-posts-list', 1468, 700, true );
...
if (get_option('wntr_show_search') == 'yes') :
	$classes[] = 'search';
	else:
	$classes[] = 'no-serach';
endif;
...
/* for escaping i.e wp_kses( __()); the html element update to WP 5.5.1 */
