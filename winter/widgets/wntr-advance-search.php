...
/**
 * Webibazaar
 * @copyright  Copyright (c) 2021 Webibazaar. (https://www.webibazaar.com)
...
class wtAdvanceSearch extends WP_Widget
{
			...
			<div class="product-search-widget input-group">
				...
				<?php  	
							...
							$all_categories = get_categories( $args ); ?>
								<option value="<?php echo esc_attr_e( '', 'branda' );?>" selected="selected"><?php echo esc_attr_e( 'All Categories', 'branda' ); ?></option>
				...
				<input type="search" class="search-field form-control" placeholder="Поиск по товарам" value="<?php echo esc_attr(get_search_query()); ?>" name="s" title="<?php echo esc_attr_x( 'Search for:', 'label', 'branda' ); ?>" />
				<input type="hidden" name="post_type" value="product" />
				<button type="submit" value="<?php echo esc_attr_x( 'Search', 'submit button', 'branda' ); ?>">Искать</button>
				</div>
    ...
    function form($instance){
		...
		?>
			<p><label for="<?php echo esc_attr($this->get_field_id('title'));?>"><?php esc_html_e('Title:', 'branda'); ?></label><input class="widefat" id="<?php echo esc_attr($this->get_field_id('title'));?>" name="<?php echo esc_attr($this->get_field_name('title'));?>" type="text" value="<?php echo esc_attr($title);?>" /></p>
	<?php }
}
